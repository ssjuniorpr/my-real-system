...
import { ArrowDownRight, ArrowUpRight, Plus, X, TrendingUp } from "lucide-react";
...
function Financeiro() {
  ...
  const { income, expense, balance } = useMemo(() => {
    ...
    txns.forEach((t) => (t.amount > 0 ? (income += t.amount) : (expense += -t.amount)));
    ...
    <div>
      ...
      <section className="px-5 mt-2 animate-fade-up">
        <div className="glass-strong rounded-3xl p-5 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="text-[10px] tracking-[0.4em] uppercase text-primary">Saldo do Mês</div>
            ...
            <div className="mt-4 h-2 rounded-full bg-secondary/50 overflow-hidden flex">
              <div
                ...
                style={{ width: `${(income / (income + expense)) * 100}%`, background: "var(--success)", boxShadow: "0 0 12px var(--success)" }}
              />
              <div
                className="h-full transition-all"
                style={{ width: `${(expense / (income + expense)) * 100}%`, background: "var(--warning)", boxShadow: "0 0 12px var(--warning)" }}
              />
            </div>
          </div>
        </div>
      ...
      <section className="px-5 mt-6 animate-fade-up">
        <div className="flex items-end justify-between mb-3">
          ...
          <span className="text-[10px] text-muted-foreground">{txns.length} registros</span>
        </div>
        <div className="space-y-2.5 pb-2">
          {txns.map((t) => (
            <TxnRow key={t.id} t={t} maxBar={maxBar} />
          ))}
        </div>
      </section>

      <FloatingAddButton onClick={() => setOpen(true)} />
      {open && <AddTxnSheet onClose={() => setOpen(false)} />}
    </div>
  );
}
...
function TxnRow({ t, maxBar }: { t: Transaction; maxBar: number }) {
...
function AddTxnSheet({ onClose }: { onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-background/70 backdrop-blur-sm animate-fade-up" onClick={onClose}>
      <div className="w-full max-w-[480px] glass-strong rounded-t-3xl p-5 pb-8 animate-fade-up" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display font-bold tracking-[0.2em] uppercase text-sm">Nova Movimentação</h3>
